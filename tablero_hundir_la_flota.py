...
def colocar_barcos_con_registro(tablero, barcos_config):
    ...
    for nombre, longitud in barcos_config.items():
        ...
        while not colocado:
            ...
            f, c = random.randint(0, len(tablero)-1), random.randint(0, len(tablero)-1)
            
            if es_posicion_valida(tablero, f, c, longitud, orientacion):
                coords_barco = []
                for i in range(longitud):
                    r, nc = (f + i, c) if orientacion == "V" else (f, c + i)
                    tablero[r][nc] = SIMBOLO_BARCO
